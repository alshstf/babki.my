...
type PositionInBase struct {
	...
	// MarketValueMinor The position's market valuation in currency at TODAY's fx rate — the one figure in this object that is current rather than historical, since it answers what the holding is worth now. IT IS CONVERTED ONCE, OUT OF THE CURRENCY THE VALUATION IS REALLY DENOMINATED IN: Position.market_value_source_minor at the rate of Position.market_value_source_currency when the row had one to convert (a bond priced off a foreign face value), and Position.market_value_minor at the rate of the position's own currency otherwise. IT IS THEREFORE NOT Position.market_value_minor TIMES THIS OBJECT'S RATE whenever those source fields are set: that figure had already been rounded to a whole minor unit of a currency this answer does not mention, and multiplying it again carried the rounding error along — 1 000,00 € routed through the dollar came out 9 999 990 kopecks where the euro rate alone gives 10 000 000. A reader who multiplies the two published figures together will land within one minor unit of this one and not on it; the money is this one, struck from one multiplication and one rounding. Never fabricated: null when Position.market_value_minor itself is null (no usable quote), AND null whenever market_value_currency differs from the position's own currency — a valuation that could not be brought into the position's currency is not carried into the base one either. That last rule is a deliberate boundary, not an arithmetic impossibility: since the conversion now starts from the valuation's own currency, a valuation stuck in EUR could be expressed in RUB whenever a EUR->RUB rate exists, with no EUR->USD rate involved at all. It is withheld so that a holding is valued in both currencies or in neither, and the display-currency switch can never turn a row showing no profit at all into a row showing one. It is the only figure here that today's rate values, so `rate_on` — the date of that rate — is null whenever this is
	MarketValueMinor nullable.Nullable[int64] `json:"market_value_minor,omitempty"`
