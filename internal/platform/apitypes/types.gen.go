...
const (
	...
	OperationSourceRegistry OperationSource = "registry"
...
const (
	...
	OperationTypeExchangeIn   OperationType = "exchange_in"
	OperationTypeExchangeOut  OperationType = "exchange_out"
	...
	OperationTypeSpinoffIn    OperationType = "spinoff_in"
	OperationTypeSpinoffOut   OperationType = "spinoff_out"
...
func (e OperationType) Valid() bool {
	...
	case OperationTypeDividend:
		return true
	case OperationTypeExchangeIn:
		return true
	case OperationTypeExchangeOut:
		return true
	...
	case OperationTypeSell:
		return true
	case OperationTypeSpinoffIn:
		return true
	case OperationTypeSpinoffOut:
		return true
...
// Defines values for TradingModeKind.
const (
	TradingModeKindNegotiated  TradingModeKind = "negotiated"
	TradingModeKindOffExchange TradingModeKind = "off_exchange"
	TradingModeKindOrderBook   TradingModeKind = "order_book"
	TradingModeKindUnknown     TradingModeKind = "unknown"
)

// Valid indicates whether the value is a known member of the TradingModeKind enum.
func (e TradingModeKind) Valid() bool {
	switch e {
	case TradingModeKindNegotiated:
		return true
	case TradingModeKindOffExchange:
		return true
	case TradingModeKindOrderBook:
		return true
	case TradingModeKindUnknown:
		return true
	default:
		return false
	}
...
type Operation struct {
	...
	TradingModeKind nullable.Nullable[TradingModeKind]    `json:"trading_mode_kind,omitempty"`
...
// TinvestExplainRequest Account for one or more of a linked account's broker rows with one manual journal operation.
//
...
type TinvestExplainRequest struct {
	// ContentKeys The `content_key` of each mirror row this operation accounts for, from TinvestUnparsedOperation. A key that is not one of this link's rows is a 404 naming it; a row already explained is a 409.
	ContentKeys []string `json:"content_keys"`

	// Operation The manual operation, in the journal's own create shape and validated by the journal's own rules — the engine replays the account with it exactly as it would for an operation entered on the journal screen, and refuses it in exactly the same cases with exactly the same messages.
	//
	...
	Operation CreateOperationRequest `json:"operation"`
}

// TinvestExplanationRemoved defines model for TinvestExplanationRemoved.
type TinvestExplanationRemoved struct {
	// SyncQueued Whether a sync was queued to project the freed rows again. False is not a failure — see TinvestExplanationResponse.sync_queued.
	SyncQueued bool `json:"sync_queued"`
}

// TinvestExplanationResponse defines model for TinvestExplanationResponse.
type TinvestExplanationResponse struct {
	// OperationId The manual operation that was created.
	OperationId openapi_types.UUID `json:"operation_id"`

	// SyncQueued Whether a sync was queued to rebuild the journal with this explanation applied. FALSE IS NOT A FAILURE and the explanation is written either way: a connection that is switched off or waiting for a new token cannot sync at all, and the explanation takes effect the next time it runs. It is also false when a sync for this connection was already queued — that run will apply it.
	SyncQueued bool `json:"sync_queued"`
}
...
type TinvestReconcileMismatch struct {
	...
	// BrokerCurrency The currency the broker's passport says the position is denominated in, ISO-4217 uppercase (the server uppercases the broker's lowercase code) — the shape CreateInstrumentRequest.currency requires. Null exactly when the passport was not obtained, or when it named no currency.
	BrokerCurrency nullable.Nullable[string] `json:"broker_currency,omitempty"`

	// BrokerIsin The ISIN the BROKER'S OWN PASSPORT gives a position nothing of ours matched — set on `unknown_security` rows alone, and there only when the check obtained the passport (it asks the broker what each unmatched position is, to pair one paper listed on two venues; see the server's matchByISIN) and the passport carried an ISIN. Null on every row of another kind; null on an unknown-security row when the broker would not say what its own position is (404), when the position lay past the number of passports one check looks up, or when the passport named no ISIN; and null on every row of a run recorded before this field existed, whose stored differences carry no such key. A value here is the one thing a catalog row can be made from that the check will then pair the position with: the pairing is by ISIN and nothing else.
	BrokerIsin nullable.Nullable[string] `json:"broker_isin,omitempty"`

	// BrokerName The name the broker's passport gives that same position — the words the reader will recognize where the label (the broker's ticker) says only «TECH2». Null exactly when the passport was not obtained, or when it named nothing; independent of `broker_isin`, since a passport can carry a name and no ISIN.
	BrokerName nullable.Nullable[string] `json:"broker_name,omitempty"`

	// BrokerType What kind of instrument the position is, ALREADY TRANSLATED INTO THIS CATALOG'S OWN InstrumentType by the importer's one table of broker types (share, bond, etf) — the same table that books every imported operation and that decides a row is `unknown_security` rather than `unsupported` in the first place. Set on `unknown_security` rows, from the position's own type rather than the passport's, so it is present even when the passport was not obtained; null on every other kind. Published translated so that a client creating the catalog row does not keep a second copy of that table, which is exactly the pair of independent computations of one thing this codebase has watched drift.
	BrokerType nullable.Nullable[InstrumentType] `json:"broker_type,omitempty"`
...
// TinvestRowExplanation The manual journal operation that stands for a broker row this program cannot read as itself — a corporate action, which the broker sends no operation type for at all.
//
...
type TinvestRowExplanation struct {
	// Id The explanation's own id — what DELETE /api/v1/tinvest/explanations/{explanationId} takes.
	Id openapi_types.UUID `json:"id"`

	// OperationId The manual journal operation. Deleting it from the journal screen removes this explanation too, and the rows it covered go back to being projected on the next sync.
	OperationId openapi_types.UUID `json:"operation_id"`

	// OperationOn The operation's journal date, which need not be either broker row's date: one operation may stand for rows a fortnight apart.
	OperationOn openapi_types.Date `json:"operation_on"`
...
// TinvestUnparsedOperation One broker operation the projection could not turn into journal entries — OR one the owner has accounted for by hand, which it could have and deliberately did not. Rows the broker has since stopped returning stay on this list: they are still operations this program could not read, and dropping them would be the silence the list exists to replace.
//
...
type TinvestUnparsedOperation struct {
	...
	// THIS IS EVIDENCE, NOT A TAX VERDICT. Whether a security is «обращающаяся» is a property of the SECURITY — admitted to trading and quoted (НК РФ ст. 214.1 п. 9) — and not of the venue of one deal, so a client must never render `off_exchange` as «необращающаяся бумага». It is a flag saying which lots will need looking at when a tax report is built, and no more than that.
	// Null exactly when `trading_mode` is null: nobody said where the operation happened, so there is no kind to name.
	TradingModeKind nullable.Nullable[TradingModeKind] `json:"trading_mode_kind"`
}
...
// ExplainTinvestRowsJSONRequestBody defines body for ExplainTinvestRows for application/json ContentType.
type ExplainTinvestRowsJSONRequestBody = TinvestExplainRequest
