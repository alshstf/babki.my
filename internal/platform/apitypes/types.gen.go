...
type Position struct {
	...
	// MarketValueGap Why this row's market valuation carries no base-currency figure, or null when it carries one — and null too when the row has no valuation at all (`market_value_minor` null), since nothing was withheld from such a row and there is no cell to put a caption on. Set where the conversion actually failed, not by comparing `market_value_currency` with `currency` afterwards, and the server's own decision to withhold `in_base.market_value_minor` reads this same answer — so the flag and the missing figure cannot come apart. It is published even when `in_base` is absent altogether, and even when `currency` already equals the base currency: the valuation cell is unconverted for a reason of its own whatever else the row is or is not missing. A SECOND FIELD rather than one more value of `in_base_gap`, because the two answer for different cells of the same row and can be true at once — a position whose basis holds a dateless lot AND whose valuation is stuck in a third currency publishes `in_base_gap: undated_lot`, which is true of the cost, the income and the profit, together with `market_value_gap: no_rate_valuation_currency`, which is true of the valuation and is the only one of the two that is. A single field would have to pick one, and either pick would caption half the row with something false. See PositionInBase.market_value_minor for why the absent base-currency valuation is an absence rather than something kept back.
