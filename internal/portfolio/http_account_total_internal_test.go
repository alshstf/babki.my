...
func TestAccountTotalCountsAHoldingThatDoesNotKnowWhatItCost(t *testing.T) {
	...
	if err := at.addPosition(pos("RUB", "10", 0, minor(120_000), minor(0), false), nil, inBaseSameCurrency); err != nil {
...
func TestAccountTotalDoesNotCallASoldOutPositionCostless(t *testing.T) {
	at := newAccountTotals("RUB")
	if err := at.addPosition(pos("RUB", "0", 0, minor(49_500), minor(49_500), false), nil, inBaseSameCurrency); err != nil {
		t.Fatalf("addPosition: %v", err)
	}
...
func TestAccountTotalRowContribution(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			at := newAccountTotals("RUB")
			if err := at.addPosition(c.p, nil, inBaseSameCurrency); err != nil {
				t.Fatalf("addPosition: %v", err)
			}
			got := at.result()
