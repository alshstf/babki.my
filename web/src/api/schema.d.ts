...
export interface components {
    schemas: {
        ...
        InstrumentEvent: {
            ...
            note: string;
            /** @description Whether this program carries this KIND into journals — true for all three today. It was the field that told a split apart from a conversion and a spin-off, which were recorded but not applied; both are applied now, as a pair of journal rows each. It is still published per row rather than left to the client to derive from `kind`, because a kind will one day be recordable before it is applicable and must be able to say so here rather than in a list every screen has to be taught. It says nothing about whether THIS event produced a row — an account that held nothing on the day gets none, and `not_counted_reason` names the one condition that stops an event of a supported kind from producing anything at all. */
            materialized: boolean;
            /** @description Why this recorded event puts nothing in any journal. ABSENT — not null — when nothing stands in its way: the field is omitted from the object entirely, so a client checks whether it is there rather than comparing it with null. It is about THIS event, where `materialized` is about its kind. Its absence does not promise that rows exist: an event of a supported kind with nothing in its way still writes nothing into an account that held none of the paper on the day, which is a fact about the account rather than about the event. The one value it can take is `result_not_in_catalog`, and it is a waiting state rather than a failure — the registry deliberately records what happened to papers this instance does not hold, and cataloguing the paper makes the next materialization write the pair. */
            not_counted_reason?: components["schemas"]["InstrumentEventNotCountedReason"];
            /** Format: date-time */
            created_at: string;
        };
        /**
         * @description Why an event of a kind this program applies still puts nothing in a journal. `result_not_in_catalog`: the conversion or spin-off produces a paper the catalog has no row for, and a journal row cannot point at a paper that is not there. Nothing is wrong and nothing was refused — the fact is recorded and waits; adding the paper to the catalog is what completes it. A split can never carry this: it produces no second paper.
         * @enum {string}
         */
        InstrumentEventNotCountedReason: "result_not_in_catalog";
