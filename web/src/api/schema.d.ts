...
export interface components {
    schemas: {
        ...
        InstrumentEvent: {
            ...
            /** @description Why this recorded event puts nothing in any journal, or null when nothing stands in its way. It is about THIS event, where `materialized` is about its kind. Null does not promise that rows exist: an event of a supported kind with nothing in its way still writes nothing into an account that held none of the paper on the day, which is a fact about the account. The one value it can take is `result_not_in_catalog`, and it is a waiting state rather than a failure — the registry deliberately records what happened to papers this instance does not hold, and cataloguing the paper makes the next materialization write the pair. */
